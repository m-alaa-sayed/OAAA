...
export class OaaaqaAgGridComponent implements OnInit, OnChanges {
    ...
    onGridReady(params: any) {
        ...
        // Do NOT call sizeColumnsToFit() here, as it forces all columns to resize 
        // to fill the entire grid width, overriding manually set column widths.
        // If precise column widths (e.g., width: 150) are desired, leave this commented out.
    ...
    private getColumnDefs(columns: any[]) {
        ...
        const translatedColumns = columns.map(column => {
            const translatedHeader = this.translate.instant(column.headerName);
            ...
            if (hasDropdownFilter && hasDropdownFilter.length > 0) {
                // Filter options to only include values that exist in the data
    ...
    private updatePrintableColumnDefs(): void {
        this._printableColumnDefs = this.columnDefs.filter(col => {
            // Exclude action columns, checkbox columns, drag columns, and row number columns
            if (!col.field) {
                return false; // Skip columns without field (usually checkbox, drag, row numbers)
            }
