...
export class DeletionResultComponent implements OnInit {
  ...
  openDetails(row: any, mode: string) {
   this.router.navigate(['/jawda/external-reviewers-deletion/request-details', row.data.id]);
  }

  navigateToNewOrder() {
    this.router.navigate(['/jawda/external-reviewers-deletion/new-order-deletion', this.module]);
  }
}
