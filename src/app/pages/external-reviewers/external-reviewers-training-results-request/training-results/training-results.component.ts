...
export class TrainingResultsComponent implements OnInit {
  ...
  permissionErTrainingResultsRegistrationRequestCreateRequest!: Permission;
  ...
  ngOnInit(): void {
    ...
    this.permissionErTrainingResultsRegistrationRequestCreateRequest = 
      this.module == 'CSEQA' ? Permission.CSEQA_ER_Training_results_Registration_Request_CREATE_REQUEST : 
      this.module == 'CHEQA' ? Permission.CHEQA_ER_Training_results_Registration_Request_CREATE_REQUEST : 
  ...
  private prepareGridHeaderCols() {
    ...
    this.actions = [
      { label: 'تفاصيل', icon: 'ri-eye-fill', callback: (row: any) => this.openDetails(row, 'view'),
        show: () => 
          this.module == 'CSEQA' ? userPermissions.includes(Permission.CSEQA_ER_Training_results_Registration_Request_VIEW) : 
          this.module == 'CHEQA' ? userPermissions.includes(Permission.CHEQA_ER_Training_results_Registration_Request_VIEW) : 
          this.module == 'OQF' ? userPermissions.includes(Permission.OQF_ER_Training_results_Registration_Request_VIEW) : false
      }
    ];
  }
  ...
  openDetails(row: any, mode: string) {
   this.router.navigate(['/jawda/external-reviewers/external-reviewers-training-results/request-details', row.data.id]);
  }

  navigateToNewOrder() {
    this.router.navigate(['/jawda/external-reviewers/external-reviewers-training-results/new-order-training-results', this.module]);
  }
