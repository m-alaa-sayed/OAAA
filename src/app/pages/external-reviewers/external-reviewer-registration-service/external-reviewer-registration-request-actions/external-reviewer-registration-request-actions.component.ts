...
export class ExternalReviewerRegistrationRequestActionsComponent extends BaseModal {
    ...
    hasApprovalPermission(): boolean {
        ...
        const requiredPermission = this.getRequiredApprovalPermission();
        return userPermissions.includes(requiredPermission);
    }

    private getRequiredApprovalPermission(): Permission {
        const module = this.requestObject.externalReviewersRegistrationRequestInfo.module;
        
        if (this.isRejoinRequest) {
            switch (module) {
                case 'CHEQA':
                    return Permission.CHEQA_ER_APPROVE_REJOIN_REQUEST;
                case 'CSEQA':
                    return Permission.CSEQA_ER_APPROVE_REJOIN_REQUEST;
                case 'OQF':
                    return Permission.OQF_ER_APPROVE_REJOIN_REQUEST;
                default:
                    return Permission.CHEQA_ER_APPROVE_REJOIN_REQUEST;
            }
        } else {
            switch (module) {
                case 'CHEQA':
                    return Permission.CHEQA_ER_APPROVE_JOIN_REQUEST;
                case 'CSEQA':
                    return Permission.CSEQA_ER_APPROVE_JOIN_REQUEST;
                case 'OQF':
                    return Permission.OQF_ER_APPROVE_JOIN_REQUEST;
                default:
                    return Permission.CHEQA_ER_APPROVE_JOIN_REQUEST;
            }
        }
