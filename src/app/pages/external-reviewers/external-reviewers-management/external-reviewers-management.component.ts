...
export class ExternalReviewersManagementComponent extends BaseModal {
  ...
  private prepareGridHeaderCols() {
    ...
    this.actions = [
      { label: this.translate.instant('PAGES.COMMON.LABELS.DETAILS'), icon: 'ri-eye-fill',link: (row: any) => `/jawda/external-reviewers/external-reviewer-details/${row.data.id}`, callback: (row: any) => this.openDetails(row.data.id) },
      { label: this.translate.instant('PAGES.COMMON.LABELS.ACTIVE'), icon: 'ri-checkbox-circle-fill', show: (row: any) => row.data.status != 'ACTIVE' && row.data.status != 'REMOVED', callback: (row: any) => this.setAction(row.data.id, "ACTIVE") },
      { label: this.translate.instant('PAGES.COMMON.LABELS.INACTIVE'), icon: 'ri-close-circle-fill', show: (row: any) => row.data.status != 'IN_ACTIVE' && row.data.status != 'WITHDRAW' && row.data.status != 'REMOVED', callback: (row: any) => this.setAction(row.data.id, "IN_ACTIVE") }
    ];
  }
