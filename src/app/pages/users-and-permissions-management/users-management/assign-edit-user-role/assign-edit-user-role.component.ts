...
export class AssignEditUserRoleComponent implements OnInit {
  ...
  private mapProceduresFromRole(role: any): ProcedureUI[] {
    ...
    return role.procedures.map((proc: any) => ({
      ...
      permissions: (proc.permissions || []).map((perm: any) => ({
        ...
        checked: perm.isMandatory ? true : (perm.enabled || false),
  ...
  onSave(): void {
    ...
    if (this.isEditMode && this.editingUserRole) {
      ...
      // Add mode: create new userRole with role reference including group
      userRole = {
        role: {
          ...
          group: this.selectedGroup ? {
            ...
            nameAr: this.selectedGroup.nameAr,
            nameEn: this.selectedGroup.nameEn
          } : undefined
        } as any,
        isPermanent: this.isPermanent,
        startDate: this.formatDateToISO(this.validityStartDate),
        ...
        this.usersManagementService.getUserById(this.userId, 'USER_ROLE').subscribe({
          next: (response) => {
            if (response?.data) {
              ...
              // Ensure group and groupId are set on the user payload
              // The backend requires these fields; use selectedGroup if the user doesn't already have one
              if (!updatedUser.group && this.selectedGroup) {
                updatedUser.group = {
                  id: this.selectedGroup.id,
                  code: this.selectedGroup.code,
                  nameAr: this.selectedGroup.nameAr,
                ...
                updatedUser.groupId = this.selectedGroup.id;
              } else if (updatedUser.group && !updatedUser.groupId) {
                updatedUser.groupId = updatedUser.group.id;
              }
