...
import { DashboardCseqaCountryDistributionResponse } from 'src/app/core/models/dashboard-cseqa.model';
...
@Component({
  selector: 'app-cseqa-country-distribution-card',
  templateUrl: './cseqa-country-distribution-card.component.html'
})
export class CseqaCountryDistributionCardComponent implements OnInit {
  page = 0;
  pageSize = 10;
  ...
  async exportPdf(): Promise<void> {
    ...
    try {
      await this.dashboardPrintExportService.printCard({
        sourceSelector: 'app-cseqa-country-distribution-card',
        reportTitleKey: 'PAGES.DASHBOARD.CSEQA.LABELS.CARD_COUNTRY_DISTRIBUTION',
  ...
  async exportExcel(): Promise<void> {
    ...
    try {
      this.customExcelExportService.exportDashboardTableWithFilters({
        fileName: `cseqa-country-distribution-${new Date().toISOString().slice(0, 10)}`,
        ...
        sheetName: 'CSEQA Country Distribution'
      });
    } finally {
      this.isExporting = false;
    }
  ...
  private load(): void {
    this.dashboardService.getCseqaCountryDistribution(this.page, this.pageSize, this.sortDirection).subscribe({
      next: (response: DashboardCseqaCountryDistributionResponse) => {
