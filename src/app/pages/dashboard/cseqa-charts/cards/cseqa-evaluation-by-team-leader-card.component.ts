...
import {
  ...
  DashboardCseqaTeamLeaderEvaluationDistributionRow
...
@Component({
  selector: 'app-cseqa-evaluation-by-team-leader-card',
  templateUrl: './cseqa-evaluation-by-team-leader-card.component.html'
})
export class CseqaEvaluationByTeamLeaderCardComponent implements OnInit {
  ...
  tableData: DashboardCseqaTeamLeaderEvaluationDistributionRow[] = [];

  columns = [
    { field: 'rating', headerName: 'درجة التقييم', cellStyle: { textAlign: 'center' } },
    {
    ...
    { headerName: 'مراجع دولي', children: [{ field: 'international', headerName: 'دولي', cellStyle: { textAlign: 'center' } }] },
  ...
  private load(): void {
    this.dashboardService.getCseqaTeamLeaderEvaluationDistribution().subscribe({
      next: (response: DashboardCseqaTeamLeaderEvaluationDistributionResponse) => {
        const rows = response?.rows ?? [];
        this.tableData = rows.map((row) => ({
          ...row,
          rating: this.helper.translateEvaluationRating(row.rating)
        }));
        this.categories = (response?.chart?.categories ?? []).map((category) => this.helper.translateEvaluationRating(category));
