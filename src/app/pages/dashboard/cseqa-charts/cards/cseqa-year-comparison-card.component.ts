...
import { DashboardCseqaYearComparisonResponse } from 'src/app/core/models/dashboard-cseqa.model';
...
@Component({
  selector: 'app-cseqa-year-comparison-card',
  templateUrl: './cseqa-year-comparison-card.component.html'
})
export class CseqaYearComparisonCardComponent implements OnInit {
  previousYear = 2025;
  currentYear = 2026;
  reviewerType = '';
  region = '';
  ...
  async exportPdf(): Promise<void> {
    ...
    try {
      await this.dashboardPrintExportService.printCard({
        sourceSelector: 'app-cseqa-year-comparison-card',
        reportTitleKey: 'PAGES.DASHBOARD.CSEQA.LABELS.CARD_YEAR_COMPARISON',
  ...
  async exportExcel(): Promise<void> {
    ...
    try {
      ...
      this.customExcelExportService.exportDashboardTableWithFilters({
        fileName: `cseqa-year-comparison-${new Date().toISOString().slice(0, 10)}`,
        headers,
        rows,
        filters,
  ...
  private load(): void {
    this.dashboardService
      .getCseqaYearComparison(this.previousYear, this.currentYear, this.reviewerType || undefined, this.region || undefined)
      .pipe(finalize(() => this.updateYearHeaders()))
      .subscribe({
        next: (response: DashboardCseqaYearComparisonResponse) => {
