...
import { DashboardOqfEqaActivitiesResponse } from 'src/app/core/models/dashboard-oqf-eqa-activities.model';
...
@Component({
  selector: 'app-oqf-activities-card',
  templateUrl: './oqf-activities-card.component.html'
})
export class OqfActivitiesCardComponent implements OnInit {
  ...
  async exportPdf(): Promise<void> {
    ...
    try {
      await this.dashboardPrintExportService.printCard({
        sourceSelector: 'app-oqf-activities-card',
        reportTitleKey: 'PAGES.DASHBOARD.OQF.LABELS.CARD_ACTIVITIES',
  ...
  async exportExcel(): Promise<void> {
    ...
    try {
      ...
      this.customExcelExportService.exportDashboardTableWithFilters({
        fileName: `oqf-activities-${new Date().toISOString().slice(0, 10)}`,
        headers: [
        ...
        filters: [{ label: this.helper.translateKey('PAGES.DASHBOARD.OQF.LABELS.YEAR'), value: yearValue }],
        sheetName: 'OQF Activities'
      });
    } finally {
      this.isExporting = false;
    }
  ...
  private load(): void {
    this.dashboardService.getOqfEqaActivitiesParticipation(this.selectedYear ?? undefined).subscribe({
      next: (response: DashboardOqfEqaActivitiesResponse) => {
