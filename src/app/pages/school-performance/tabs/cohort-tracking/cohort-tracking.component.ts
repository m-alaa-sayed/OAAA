...
import { CohortData, NotApplicableKey, SchoolInfo } from '../../types/school-info';
import { TranslateService } from '@ngx-translate/core';
import { ToastService } from 'src/app/core/services/toast-service';

@Component({
  selector: 'app-cohort-tracking',
  templateUrl: './cohort-tracking.component.html',
  styleUrls: ['./cohort-tracking.component.scss']
})
export class CohortTrackingComponent implements OnInit, OnDestroy, OnChanges {
  ...
  gradesNotApplicableMap: { key: string; label: NotApplicableKey }[] = [
    { key: '5th', label: '_5thNotApplicable' },
    { key: '6th', label: '_6thNotApplicable' },
    { key: '7th', label: '_7thNotApplicable' },
    { key: '8th', label: '_8thNotApplicable' },
    { key: '9th', label: '_9thNotApplicable' },
    { key: '10th', label: '_10thNotApplicable' },
    { key: '11th', label: '_11thNotApplicable' },
    { key: '12th', label: '_12thNotApplicable' }
  ];
  ...
  getAverageForGrade(grade: string, year: string): number {
    ...
    const values = rows.map(r => r[year])?.filter(v => v !== null  && v > 0) as number[];
  ...
  private updateSubjectAverages(grade: string, year: string): void {
    ...
    for (const avgSubject of Object.keys(subjectsMapping)) {
      ...
      const values = group.data
        ...
        .map((r: { [x: string]: any; }) => r[year] ?? 0).filter((v: number) => v > 0); 
  ...
  onNotApplicableChange(key: NotApplicableKey, event: any) {
    const mapItem = this.gradesNotApplicableMap.find(g => g.key === key);
    if (mapItem) {
      this.schoolInfo[mapItem.label] = event.target.checked;
    }
  }

  isNotApplicable(gradeKey: any): boolean {
    const mapItem = this.gradesNotApplicableMap.find(g => g.key === gradeKey);
    if (mapItem) {
      return this.schoolInfo[mapItem.label] || false;
    }
    return false;
  }
