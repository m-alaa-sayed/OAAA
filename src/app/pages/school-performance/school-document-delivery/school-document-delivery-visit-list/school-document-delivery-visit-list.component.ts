...
export class SchoolDocumentDeliveryVisitListComponent extends BaseModal implements OnInit {
    ...
    private prepareGridHeaderCols() {
        ...
        const download = {
            ...
            show: () => userPermissions.includes(Permission.SCHOOL_DOCUMENT_DELIVERY_EXPORT)
        };

        const selectVisit = {
            label: t('PAGES.COMMON.LABELS.VIEW_DETAILS'),
            icon: 'ri-eye-fill',
            callback: (row: any) => this.openVisit(row),
            show: () => userPermissions.includes(Permission.SCHOOL_DOCUMENT_DELIVERY_VIEW_DETAILS)
        };
