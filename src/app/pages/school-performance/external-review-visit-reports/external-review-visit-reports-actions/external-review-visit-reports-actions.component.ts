...
export class ExternalReviewVisitReportsActionsComponent extends BaseModal {
    ...
    @Output() validateBeforeOpenEmitter = new EventEmitter<{action: string, content: any}>();
    ...
    setAction(action: any, content: any) {
        ...
        if (this.handleSpecialErrorCases(action)) {
            // For actions that require validation, validate before opening popup
            if (action === 'SUBMIT' || action === 'APPROVE' || action === 'RETURN_FOR_EDIT' || action === 'SEND_TO_PROOFREADER' || action === 'SEND_TO_GM') {
                this.validateBeforeOpenEmitter.emit({action: action, content: content});
            } else {
                // For other actions, open popup directly
                this.comment = "";
                this.open(content);
            }
        }
