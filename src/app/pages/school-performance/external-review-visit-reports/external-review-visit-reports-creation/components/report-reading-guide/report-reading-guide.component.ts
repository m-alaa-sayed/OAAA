...
export class ReportReadingGuideComponent extends BaseModal {
    ...
    validate(content: any) {
        // Force change detection to ensure all form values are updated
        this.cdr.detectChanges();
        
        // Use setTimeout to ensure all two-way bindings are processed
        setTimeout(() => {
            // Pass 'SUBMIT' action to validate summary fields only for submit
            if (!this.externalReviewVisitReportsWizaredService.isValid(this.visitReportSubmissionRequestInfo, false, 'SUBMIT')) {
                return;
            }
        this.open(content);
        }, 100);
